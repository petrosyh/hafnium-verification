...
(* The following definitions are old versions that I have to remove later. 
...
Section FLAG_TO_VALUE_and_VALUE_TO_FLAG.
  ...
  Definition MMATTR_SHAREABLE_to_Z (flag: MMATTTR_SHAREABLE) : Z :=
    match flag with
    | NON_SHAREABLE => Int64.unsigned Constant.NON_SHAREABLE
    | OUTER_SHAREABLE => Int64.unsigned Constant.OUTER_SHAREABLE
    | INNER_SHAREABLE => Int64.unsigned Constant.INNER_SHAREABLE
    end.

  Definition Z_to_MMATTR_SHAREABLE (value: Z) : option MMATTTR_SHAREABLE :=
    if zeq value (Int64.unsigned Constant.NON_SHAREABLE) then Some NON_SHAREABLE
    else if zeq value (Int64.unsigned Constant.OUTER_SHAREABLE) then Some OUTER_SHAREABLE
         else if zeq value (Int64.unsigned Constant.INNER_SHAREABLE) then Some INNER_SHAREABLE
              else None.

  Definition MMATTR_PTE_VALID_to_Z (flag: MMATTR_PTE_VALID) : Z :=
    match flag with
    | PTE_VALID => Int64.unsigned Constant.PTE_VALID
    | PTE_INVALID => 0
    end.

  Definition Z_to_MMATTR_PTE_VALID (value: Z) : option MMATTR_PTE_VALID :=
    if zeq value 0 then Some PTE_INVALID
    else if zeq value (Int64.unsigned Constant.PTE_VALID) then Some PTE_VALID
         else None.
  

  Definition MMATTR_PTE_TABLE_to_Z (flag: MMATTR_PTE_TABLE) : Z :=
    match flag with
    | PTE_TABLE => Int64.unsigned Constant.PTE_TABLE
    | PTE_IS_NOT_TABLE => 0
    end.

  Definition Z_to_MMATTR_PTE_TABLE (value : Z) : option MMATTR_PTE_TABLE :=
    if zeq value 0 then Some PTE_IS_NOT_TABLE
    else if zeq value (Int64.unsigned Constant.PTE_TABLE) then Some PTE_TABLE
         else None.

  Definition MMATTR_PTE_LEVEL0_BLOCK_to_Z (flag: MMATTR_PTE_LEVEL0_BLOCK) :=
    match flag with
    | PTE_LEVEL0_BLOCK => Int64.unsigned Constant.PTE_LEVEL0_BLOCK
    | PTE_LEVEL0_NOT_BLOCK => 0
    end.
  
  Definition Z_to_MMATTR_PTE_LEVEL0_BLOCK (value : Z) : option MMATTR_PTE_LEVEL0_BLOCK :=
    if zeq value 0 then Some PTE_LEVEL0_NOT_BLOCK
    else if zeq value (Int64.unsigned Constant.PTE_LEVEL0_BLOCK) then Some PTE_LEVEL0_BLOCK
         else None.

  Definition MMATTR_STAGE1_ATTR_FLAGS_to_Z (flag: MMATTR_STAGE1_ATTR_FLAGS) :=
    match flag with
    | STAGE1_XN => (Int64.unsigned Constant.STAGE1_XN)
    | STAGE1_PXN => (Int64.unsigned Constant.STAGE1_PXN)
    | STAGE1_CONTIGUOUS => (Int64.unsigned Constant.STAGE1_CONTIGUOUS)
    | STAGE1_DBM => (Int64.unsigned Constant.STAGE1_DBM)
    | STAGE1_NG => (Int64.unsigned Constant.STAGE1_NG)
    | STAGE1_AF => (Int64.unsigned Constant.STAGE1_AF)
    | STAGE1_SH x => (Int64.unsigned (Constant.STAGE1_SH (Int64.repr x))) 
    | STAGE1_AP2 => (Int64.unsigned Constant.STAGE1_AP2)
    | STAGE1_AP1 => (Int64.unsigned Constant.STAGE1_AP1)
    | STAGE1_AP x => (Int64.unsigned (Constant.STAGE1_AP (Int64.repr x)))
    | STAGE1_NS => (Int64.unsigned Constant.STAGE1_NS)
    | STAGE1_ATTRINDX x => (Int64.unsigned (Constant.STAGE1_ATTRINDX (Int64.repr x)))
    end.

  Definition Z_to_MMATTR_STAGE1_ATTR_FLAGS (value : Z) (x : option Z) : option MMATTR_STAGE1_ATTR_FLAGS :=
    match x with
    | None =>
      if zeq value (Int64.unsigned Constant.STAGE1_XN)
      then Some STAGE1_XN
      else if zeq value (Int64.unsigned Constant.STAGE1_PXN)
           then Some STAGE1_PXN
           else if zeq value (Int64.unsigned Constant.STAGE1_CONTIGUOUS)
                then Some STAGE1_CONTIGUOUS
                else if zeq value (Int64.unsigned Constant.STAGE1_DBM)
                     then Some STAGE1_DBM
                     else if zeq value (Int64.unsigned Constant.STAGE1_NG)
                          then Some STAGE1_NG
                          else if zeq value (Int64.unsigned Constant.STAGE1_AF)
                               then Some STAGE1_AF
                               else if zeq value (Int64.unsigned Constant.STAGE1_AP2)
                                    then Some STAGE1_AP2
                                    else if zeq value (Int64.unsigned Constant.STAGE1_AP1)
                                         then Some STAGE1_AP1
                                         else if zeq value (Int64.unsigned Constant.STAGE1_NS)
                                              then Some STAGE1_NS
                                              else None
    | Some x' =>
      if zeq value (Int64.unsigned (Constant.STAGE1_SH (Int64.repr x')))
      then Some (STAGE1_SH x')
      else if zeq value (Int64.unsigned (Constant.STAGE1_AP(Int64.repr x')))
           then Some (STAGE1_AP x')
           else if zeq value (Int64.unsigned (Constant.STAGE1_ATTRINDX (Int64.repr x')))
                then Some (STAGE1_ATTRINDX x')
                else None
    end.

  Definition MMATTR_STAGE1_READWRITEPERM_to_Z (flag: MMATTR_STAGE1_READWRITEPERM) : Z :=
    match flag with
    | STAGE1_READONLY => Int64.unsigned Constant.STAGE1_READONLY
    | STAGE1_READWRITE => Int64.unsigned Constant.STAGE1_READWRITE
    end.

  Definition Z_to_MMATTR_STAGE1_READWRITEPERM (value: Z) : option MMATTR_STAGE1_READWRITEPERM :=
    if zeq value (Int64.unsigned Constant.STAGE1_READONLY)
    then Some STAGE1_READONLY
    else if zeq value (Int64.unsigned Constant.STAGE1_READWRITE)
         then Some STAGE1_READWRITE
         else None.

  Definition MMATTR_STAGE1_INDX_to_Z (flag: MMATTR_STAGE1_INDX) : Z :=
    match flag with
    | STAGE1_DEVICEINDX => Int64.unsigned Constant.STAGE1_DEVICEINDX
    | STAGE1_NORMALINDX => Int64.unsigned Constant.STAGE1_NORMALINDX
    end.

  Definition Z_to_MMATTR_STAGE1_INDX (value : Z) : option MMATTR_STAGE1_INDX :=
    if zeq value (Int64.unsigned Constant.STAGE1_DEVICEINDX)
    then Some STAGE1_DEVICEINDX
    else if zeq value (Int64.unsigned Constant.STAGE1_NORMALINDX)
         then Some STAGE1_NORMALINDX
         else None.
    
  Definition MMATTR_STAGE2_ATTR_FLAGS_to_Z (flag: MMATTR_STAGE2_ATTR_FLAGS) : Z :=
    match flag with
    | STAGE2_XN x => (Int64.unsigned (Constant.STAGE2_XN (Int64.repr x)))
    | STAGE2_CONTIGUOUS => (Int64.unsigned Constant.STAGE2_CONTIGUOUS)
    | STAGE2_DBM => (Int64.unsigned Constant.STAGE2_DBM)
    | STAGE2_AF => (Int64.unsigned Constant.STAGE2_AF)
    | STAGE2_SH x => (Int64.unsigned (Constant.STAGE2_SH (Int64.repr x)))
    | STAGE2_S2AP x => (Int64.unsigned (Constant.STAGE2_S2AP (Int64.repr x)))
    end.


  Definition Z_to_MMATTR_STAGE2_ATTR_FLAGS (value : Z) (x : option Z) : option MMATTR_STAGE2_ATTR_FLAGS :=
    match x with
    | None =>
      if zeq value (Int64.unsigned Constant.STAGE2_CONTIGUOUS)
      then Some STAGE2_CONTIGUOUS
      else if zeq value (Int64.unsigned Constant.STAGE2_DBM)
           then Some STAGE2_DBM
           else if zeq value (Int64.unsigned Constant.STAGE2_AF)
                then Some STAGE2_AF
                else None
    | Some x' =>
      if zeq value (Int64.unsigned (Constant.STAGE2_XN (Int64.repr x')))
      then Some (STAGE2_XN x')
      else if zeq value (Int64.unsigned (Constant.STAGE2_SH (Int64.repr x')))
           then Some (STAGE2_SH x')
           else if zeq value (Int64.unsigned (Constant.STAGE2_S2AP (Int64.repr x')))
                then Some (STAGE2_S2AP x')
                else None      
    end.

  Definition MMATTR_STAGE2_EXECUTE_ATTR_FLAGS_to_Z (flag : MMATTR_STAGE2_EXECUTE_ATTR_FLAGS) : Z :=
    match flag with 
    | STAGE2_EXECUTE_ALL => Int64.unsigned Constant.STAGE2_EXECUTE_ALL
    | STAGE2_EXECUTE_EL0 => Int64.unsigned Constant.STAGE2_EXECUTE_EL0
    | STAGE2_EXECUTE_NONE => Int64.unsigned Constant.STAGE2_EXECUTE_NONE
    | STAGE2_EXECUTE_EL1 => Int64.unsigned Constant.STAGE2_EXECUTE_EL1
    | STAGE2_EXECUTE_MASK => Int64.unsigned Constant.STAGE2_EXECUTE_MASK
    end.

  Definition Z_to_MMATTR_STAGE2_EXECUTE_ATTR_FLAGS (value : Z) : option MMATTR_STAGE2_EXECUTE_ATTR_FLAGS :=
    if zeq value (Int64.unsigned Constant.STAGE2_EXECUTE_ALL)
    then Some STAGE2_EXECUTE_ALL
    else if zeq value (Int64.unsigned Constant.STAGE2_EXECUTE_EL0)
         then Some STAGE2_EXECUTE_EL0
         else if zeq value (Int64.unsigned Constant.STAGE2_EXECUTE_NONE)
              then Some STAGE2_EXECUTE_NONE
              else if zeq value (Int64.unsigned Constant.STAGE2_EXECUTE_EL1)
                   then Some STAGE2_EXECUTE_EL1
                   else if zeq value (Int64.unsigned Constant.STAGE2_EXECUTE_MASK)
                        then Some STAGE2_EXECUTE_MASK
                        else None.
  

  Definition MMATTR_STAGE1_TABLE_ATTR_FLAGS_to_Z (flag : MMATTR_STAGE1_TABLE_ATTR_FLAGS) : Z :=
    match flag with
    | TABLE_NSTABLE => Int64.u7nsigned Constant.TABLE_NSTABLE
    | TABLE_APTABLE1 => Int64.unsigned Constant.TABLE_APTABLE1
    | TABLE_APTABLE0 => Int64.unsigned Constant.TABLE_APTABLE0
    | TABLE_XNTABLE => Int64.unsigned Constant.TABLE_XNTABLE
    | TABLE_PXNTABLE => Int64.unsigned Constant.TABLE_PXNTABLE
    end.

  Definition Z_to_MMATTR_STAGE1_TABLE_ATTR_FLAGS (value : Z) : option MMATTR_STAGE1_TABLE_ATTR_FLAGS :=
    if zeq value (Int64.unsigned Constant.TABLE_NSTABLE)
    then Some TABLE_NSTABLE
    else if zeq value (Int64.unsigned Constant.TABLE_APTABLE1)
         then Some TABLE_APTABLE1
         else if zeq value (Int64.unsigned Constant.TABLE_APTABLE0)
              then Some TABLE_APTABLE0
              else if zeq value (Int64.unsigned Constant.TABLE_XNTABLE)
                   then Some TABLE_XNTABLE
                   else if zeq value (Int64.unsigned Constant.TABLE_PXNTABLE)
                        then Some TABLE_PXNTABLE
                        else None.

  Definition MMATTR_STAGE2_ONWERSHIP_FLAGS_to_Z (flag : MMATTR_STAGE2_ONWERSHIP_FLAGS) : Z :=
    match flag with
    | STAGE2_SW_OWNED => Int64.unsigned Constant.STAGE2_SW_OWNED
    | STAGE2_SW_EXCLUSIVE => Int64.unsigned Constant.STAGE2_SW_EXCLUSIVE
    end.

  Definition Z_to_MMATTR_STAGE2_ONWERSHIP_FLAGS (value : Z) : option MMATTR_STAGE2_ONWERSHIP_FLAGS :=
    if zeq value (Int64.unsigned Constant.STAGE2_SW_OWNED)
    then Some STAGE2_SW_OWNED
    else if zeq value (Int64.unsigned Constant.STAGE2_SW_EXCLUSIVE)
         then Some STAGE2_SW_EXCLUSIVE
         else None.
    
  Definition MMATTR_STAGE2_NORMAL_MEMORY_ATTR_to_Z (flag : MMATTR_STAGE2_NORMAL_MEMORY_ATTR) : Z :=
    match flag with
    | STAGE2_DEVICE_MEMORY => Int64.unsigned Constant.STAGE2_DEVICE_MEMORY
    | STAGE2_NONCACHEABLE => Int64.unsigned Constant.STAGE2_NONCACHEABLE
    | STAGE2_WRITETHROUGH => Int64.unsigned Constant.STAGE2_WRITETHROUGH
    | STAGE2_WRITEBACK => Int64.unsigned Constant.STAGE2_WRITEBACK
    end.

  Definition Z_to_MMATTR_STAGE2_NORMAL_MEMORY_ATTR (value : Z) : option MMATTR_STAGE2_NORMAL_MEMORY_ATTR :=
    if zeq value (Int64.unsigned Constant.STAGE2_DEVICE_MEMORY)
    then Some STAGE2_DEVICE_MEMORY
    else if zeq value (Int64.unsigned Constant.STAGE2_NONCACHEABLE)
         then Some STAGE2_NONCACHEABLE
         else if zeq value (Int64.unsigned Constant.STAGE2_WRITETHROUGH)
              then Some STAGE2_WRITETHROUGH
              else if zeq value (Int64.unsigned Constant.STAGE2_WRITEBACK)
                   then Some STAGE2_WRITEBACK
                   else None. 

  Definition MMATTR_STAGE2_DEVICE_MEMORY_ATTR_to_Z (flag : MMATTR_STAGE2_DEVICE_MEMORY_ATTR) : Z :=
    match flag with
    | STAGE2_MEMATTR_DEVICE_nGnRnE => Int64.unsigned Constant.STAGE2_MEMATTR_DEVICE_nGnRnE
    | STAGE2_MEMATTR_DEVICE_nGnRE => Int64.unsigned Constant.STAGE2_MEMATTR_DEVICE_nGnRE
    | STAGE2_MEMATTR_DEVICE_nGRE => Int64.unsigned Constant.STAGE2_MEMATTR_DEVICE_nGRE
    | STAGE2_MEMATTR_DEVICE_GRE => Int64.unsigned Constant.STAGE2_MEMATTR_DEVICE_GRE
    end.

  Definition Z_to_MMATTR_STAGE2_DEVICE_MEMORY_ATTR (value : Z) : option MMATTR_STAGE2_DEVICE_MEMORY_ATTR :=
    if zeq value (Int64.unsigned Constant.STAGE2_MEMATTR_DEVICE_nGnRnE)
    then Some STAGE2_MEMATTR_DEVICE_nGnRnE
    else if zeq value (Int64.unsigned Constant.STAGE2_MEMATTR_DEVICE_nGnRE)
         then Some STAGE2_MEMATTR_DEVICE_nGnRE
         else if zeq value (Int64.unsigned Constant.STAGE2_MEMATTR_DEVICE_nGRE)
              then Some STAGE2_MEMATTR_DEVICE_nGRE
              else if zeq value (Int64.unsigned Constant.STAGE2_MEMATTR_DEVICE_GRE)
                   then Some STAGE2_MEMATTR_DEVICE_GRE
                   else None. 

  Definition MMATTR_STAGE2_MEMORY_ATTR_to_Z (flag : MMATTR_STAGE2_MEMORY_ATTR) : Z :=
    match flag with
    | STAGE2_MEMATTR outer inner => Int64.unsigned (Constant.STAGE2_MEMATTR (Int64.repr outer) (Int64.repr inner))
    | STAGE2_MEMATTR_TYPE_MASK => Int64.unsigned Constant.STAGE2_MEMATTR_TYPE_MASK
    end.

  Definition Z_to_MMATTR_STAGE2_MEMORY_ATTR (value : Z) (x : option (Z * Z)): option MMATTR_STAGE2_MEMORY_ATTR :=
    match x with
    | None =>
      if zeq value (Int64.unsigned Constant.STAGE2_MEMATTR_TYPE_MASK)
      then Some STAGE2_MEMATTR_TYPE_MASK
      else None
    | Some (outer, inner) =>
      if zeq value (Int64.unsigned (Constant.STAGE2_MEMATTR (Int64.repr outer) (Int64.repr inner)))
      then Some (STAGE2_MEMATTR outer inner)
      else None
    end.

  Definition MMATTR_STAGE2_READWRITEPERM_to_Z (flag : MMATTR_STAGE2_READWRITEPERM) : Z :=
    match flag with
    | STAGE2_ACCESS_READ => Int64.unsigned Constant.STAGE2_ACCESS_READ
    | STAGE2_ACCESS_WRITE => Int64.unsigned Constant.STAGE2_ACCESS_WRITE
    end.

  Definition Z_to_MMATTR_STAGE2_READWRITEPERM (value : Z) (x : option (Z * Z)): option MMATTR_STAGE2_READWRITEPERM :=
      if zeq value (Int64.unsigned Constant.STAGE2_ACCESS_READ)
      then Some STAGE2_ACCESS_READ
      else if zeq value (Int64.unsigned Constant.STAGE2_ACCESS_WRITE)
           then  Some STAGE2_ACCESS_WRITE
           else None.  
*)
