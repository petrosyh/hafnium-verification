...
Definition classify_bool (v: val) : classify_bool_cases :=
  match v with
  | Vint _ => bool_case_i
  | Vptr _ _ => bool_case_p
  | Vlong _ => bool_case_l
  | _ => bool_default
  end.
...
Definition classify_neg (v: val) : classify_neg_cases :=
  match v with
  | Vint n => neg_case_i Unsigned
  | Vlong n => neg_case_l Unsigned
  | _ => neg_default
  end.

Definition sem_neg (v: val) : option val :=
  match classify_neg v with
  | neg_case_i sg =>
      match v with
...
Definition classify_notint (v: val) : classify_notint_cases :=
  match v with
  | Vint n => notint_case_i Unsigned
  | Vlong n => notint_case_l Unsigned
  | _ => notint_default
  end.

Definition sem_notint (v: val): option val :=
  match classify_notint v with
  | notint_case_i sg =>
      match v with
...
Definition classify_binarith (v1: val) (v2: val) : binarith_cases :=
  match v1, v2 with
...
Definition classify_add (v1: val) (v2: val) :=
  match v1, v2 with
  | Vptr _ _, Vint _  => add_case_pi
  | Vptr _ _, Vlong _ => add_case_pl
  | Vint _, Vptr _ _ => add_case_ip
  | Vlong _, Vptr _ _ => add_case_lp
  | _, _ => add_default
  end.
...
Definition sem_add_ptr_int (si: signedness) (v1 v2: val): option val :=
  ...
  | Vptr b1 ofs1, Vint n2 =>
      ...
      Some (Vptr b1 (Ptrofs.add ofs1 (Ptrofs.mul (Ptrofs.repr value_size) n2)))
  | Vint n1, Vint n2 =>
      if Archi.ptr64 then None else Some (Vint (Int.add n1 (Int.mul (Int.repr value_size) n2)))
  | Vlong n1, Vint n2 =>
      let n2 := cast_int_long si n2 in
      if Archi.ptr64 then Some (Vlong (Int64.add n1 (Int64.mul (Int64.repr value_size) n2))) else None
  | _,  _ => None
  end.
...
Inductive classify_sub_cases : Type :=
  | sub_case_pi  (**r pointer, int *)
  | sub_case_pp               (**r pointer, pointer *)
  | sub_case_pl               (**r pointer, long *)
  | sub_default.                         (**r numerical type, numerical type *)

Definition classify_sub (v1: val) (v2: val) :=
  match v1, v2 with
  | Vptr _ _, Vint _ => sub_case_pi
  | Vptr _ _, Vptr _ _ => sub_case_pp
  | Vptr _ _, Vlong _ => sub_case_pl
  | _, _ => sub_default
  end.
...
Definition sem_sub (v1:val) (v2: val) : option val :=
  ...
  | sub_case_pi =>            (**r pointer minus integer *)
      match v1, v2 with
      | Vptr b1 ofs1, Vint n2 =>
          let n2 := ptrofs_of_int Unsigned n2 in
          Some (Vptr b1 (Ptrofs.sub ofs1 (Ptrofs.mul (Ptrofs.repr value_size) n2)))
      | Vint n1, Vint n2 =>
          if Archi.ptr64 then None else Some (Vint (Int.sub n1 (Int.mul (Int.repr value_size) n2)))
      | Vlong n1, Vint n2 =>
          let n2 := cast_int_long Unsigned n2 in
          if Archi.ptr64 then Some (Vlong (Int64.sub n1 (Int64.mul (Int64.repr value_size) n2))) else None
      | _,  _ => None
      end
  | sub_case_pl =>            (**r pointer minus long *)
      match v1, v2 with
      | Vptr b1 ofs1, Vlong n2 =>
          let n2 := Ptrofs.of_int64 n2 in
          Some (Vptr b1 (Ptrofs.sub ofs1 (Ptrofs.mul (Ptrofs.repr value_size) n2)))
  ...
  | sub_case_pp =>          (**r pointer minus pointer *)
      match v1,v2 with
      | Vptr b1 ofs1, Vptr b2 ofs2 =>
          if eq_block b1 b2 then
            let sz := value_size in
...
Definition classify_shift (v1: val) (v2: val) :=
  match v1, v2 with
  | Vint _, Vint _ => shift_case_ii Unsigned
  | Vint _, Vlong _ => shift_case_il Unsigned
  | Vlong _, Vint _ => shift_case_li Unsigned
  | Vlong _, Vlong _ => shift_case_ll Unsigned
  | _,_  => shift_default
  end.
...
Definition sem_shift
    ...
    (v1: val) (v2: val) : option val :=
  match classify_shift v1 v2 with
  | shift_case_ii sg =>
      match v1, v2 with
      | Vint n1, Vint n2 =>
...
Definition sem_shl (v1:val) (v2: val) : option val :=
  sem_shift
    (fun sg n1 n2 => Int.shl n1 n2)
    (fun sg n1 n2 => Int64.shl n1 n2)
    v1 v2.

Definition sem_shr (v1:val) (v2: val) : option val :=
  sem_shift
...
Definition classify_cmp (v1: val) (v2: val) :=
  match v1, v2 with
  | Vptr _ _, Vptr _ _ => cmp_case_pp
  | Vptr _ _ , Vint _ => cmp_case_pi Unsigned
  | Vint _, Vptr _ _ => cmp_case_ip Unsigned
  | Vptr _ _ , Vlong _ => cmp_case_pl
  | Vlong _ , Vptr _ _ => cmp_case_lp
  | _, _ => cmp_default
  end.
...
Definition classify_switch (v: val) :=
  match v with
  | Vint _ => switch_case_i
  | Vlong _ => switch_case_l
  | _ => switch_default
  end.

Definition sem_switch_arg (v: val): option Z :=
  match classify_switch v with
  | switch_case_i =>
...
(* Lemma sem_cast_inj: *)
(*   forall v1 ty v tv1, *)
(*   sem_cast v1 ty m = Some v -> *)
(*   Val.inject f v1 tv1 -> *)
(*   exists tv, sem_cast tv1 ty m'= Some tv /\ Val.inject f v tv. *)
(* Proof. *)
(*   unfold sem_cast; intros; destruct (classify_cast v ty); inv H0; TrivialInject. *)
(* - econstructor; eauto. *)
